...
A second external dependency handles translation (French disruption text → English): DeepL's API is primary — best FR/EN quality of the free-tier options — with automatic fallback to [MyMemory](https://mymemory.translated.net)'s free, keyless API when DeepL fails or its quota is exhausted (`apps/server/src/translate.ts`). DeepL needs its own free account and API key, separate from PRIM — the user's to create, not something that can be done on their behalf; MyMemory needs neither. **Correction to an earlier assumption here**: DeepL's free tier (1,000,000 characters/month on this account) turned out *not* to be far more than needed — an in-memory-only cache meant every redeploy wiped it, forcing a full re-translation burst on the next poll, which alone was enough to exhaust the monthly quota in days. The cache now persists to disk (see below), which should keep steady-state usage far lower going forward.
...
- **Bilingual EN/FR, fully, including live disruption text.** UI chrome (wordmark, legend, ticker labels, empty/all-clear states) uses standard static i18n. Disruption messages are dynamic French text from IDFM and are machine-translated server-side: on first sight of a new message string, the backend translates it once (DeepL first, MyMemory as a free fallback — see above) and caches the French→English pair, keyed by the original text; every later broadcast of that same message (it's typically active across many poll cycles) reuses the cached translation rather than re-translating. This mirrors the same "poll once, reuse for everyone" discipline used for the IDFM feed itself, and keeps translation-API usage proportional to the number of *distinct* messages per day, not the number of polls or visitors. The cache is written to disk (a small JSON file on a named Docker volume, not a database) specifically so a redeploy — which recreates the container from a fresh image — doesn't wipe it and force re-translating everything at once.
